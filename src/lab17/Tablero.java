...
class Tablero {
    ...
    private void moverEjercitoEnTablero(Ejercito ejercito, int direccion, Reino reino) {
        ...
        // determinamos la nueva posicion en base a la direccion
        switch (direccion) {
            case 1 -> x = Math.max(x - 1, 0); // arriba
            case 2 -> x = Math.min(x + 1, 9); // abajo
            case 3 -> y = Math.max(y - 1, 0); // izquierda
            case 4 -> y = Math.min(y + 1, 9); // derecha
        }

        // si hay un ejercito en la nueva posicion, verificamos si es enemigo
        if (tablero[x][y] != null) {
            ...
            if (enemigo != null) {
                ...
                tablero[posicion[0]][posicion[1]] = ejercito.getId(); // restauramos al ejercito en su posicion original
                return;
            }
        } else {
            // si no hay ningun ejercito en la posicion, movemos el ejercito
            tablero[x][y] = ejercito.getId();
        }
    }
    ...
    private void resolverBatalla(Ejercito ejercito, Ejercito enemigo, Reino reino) {
        ...
        if (promedioEjercito > promedioEnemigo) {
            ...
            if (posicionEnemigo != null) {
                tablero[posicionEnemigo[0]][posicionEnemigo[1]] = null; // eliminamos enemigo del tablero
            }
            (reino == reinoNorte ? reinoSur : reinoNorte).removerEjercito(enemigo); // eliminamos del reino contrario
            ...
            if (posicionEjercito != null) {
                tablero[posicionEjercito[0]][posicionEjercito[1]] = null; // eliminamos ejercito del tablero
            }
            reino.removerEjercito(ejercito); // eliminamos del reino actual
        } else {
